...
abstract class BaseRepository implements RepositoryContract
{
     ...
     * @throws \Exception
     */
    public function deleteByColumn($item, $column)
    {
        $this->newQuery()->setClauses()->setScopes();

        $result = $this->query->where($column, $item)->delete();
     ...
     * @return \Illuminate\Database\Eloquent\Builder[]|Collection
     */
    public function getAllApprovedByColumn($item, $column, array $columns = ['*'])
    {
        $this->unsetClauses();

        $this->newQuery()->eagerLoad();

        return $this->query->where($column, $item)->where('approved', 1)->get($columns);
    }
